...
class EvolutionFeedbackLoop:
    ...
    def __init__(
        ...
        # 初始化 CI/CD 集成
        self.cicd_enabled = cfg.get("evolution.cicd.enabled", False)
        self.cicd_auto_trigger = cfg.get("evolution.cicd.auto_trigger", True)
        self.cicd_branch = cfg.get("evolution.cicd.branch", "evolution")
        self.cicd_create_pr = cfg.get("evolution.cicd.create_pr", True)
        self.cicd_auto_merge = cfg.get("evolution.cicd.auto_merge", False)
        
        if self.cicd_enabled:
            self.cicd = GitHubCICDIntegration()
            if self.cicd.is_configured():
                print(f"🚀 CI/CD Integration enabled for {self.cicd.owner}/{self.cicd.repo}")
            else:
                print("⚠️ CI/CD enabled but not configured (missing GITHUB_TOKEN/OWNER/REPO)")
                self.cicd_enabled = False
        else:
            self.cicd = None
        ...
        # CI/CD 结果记录
        self.cicd_results: List[CICDResult] = []
    ...
    def run_full_cycle(self, ideas: List[CreativeIdea] = None) -> Dict[str, Any]:
        ...
        # Stage 3.5: CI/CD 自动化 (如果启用且有成功应用的想法)
        if self.cicd_enabled and results["stages"]["apply"].get("success", 0) > 0:
            results["stages"]["cicd"] = self._stage_cicd()
    ...
    def _stage_cicd(self) -> Dict:
        ...
        print(f"  自动创建 PR: {self.cicd_create_pr}")
        print(f"  自动合并: {self.cicd_auto_merge}")
        
        if not self.cicd or not self.cicd_enabled:
...
*This PR was automatically created by the Evolution Feedback Loop*"""
            ...
            # 记录结果
            self.cicd_results.append(result)
            self.stats["cicd_triggered"] += 1
            
            if result.success:
                self.stats["cicd_success"] += 1
                print(f"  ✅ CI/CD 成功!")
            ...
            return {
                ...
                "details": {
                    ...
                    "commit": result.details.get("commit"),
                    "pr_number": result.details.get("pr_number"),
                    "url": result.details.get("url") or (result.workflow_run.html_url if result.workflow_run else None)
                }
            }
